...
describe Spree::Order, type: :model do
  ...
  context 'before transition to pax checkout state' do
    before do
      ...
      # allow_any_instance_of(Spree::Context).to receive(:child).and_return('1')
      allow_any_instance_of(Spree::LineItem).to receive(:context).and_return(Spree::Context.new)
    end

    it 'should create  pax' do
      expect(order.line_items.first.paxes).to receive(:new)
      # here is missing the `times`
