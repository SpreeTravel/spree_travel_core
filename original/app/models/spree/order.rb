module Spree
  class Order < ActiveRecord::Base
    attr_accessible :line_items, :bill_address_attributes, :ship_address_attributes, :payments_attributes,
                    ...
                    :pax_contacts, :pax_contacts_attributes, :check_in, :check_out, :custom_text
    ...
    state_machine :initial => 'cart', :use_transactions => false do
      ...
      event :next do
        ...
        transition :from => 'address',  :to => 'pax'
        #transition :from => 'pax', :to => 'payment', :if => :payment_required?
        transition :from => 'pax', :to => 'complete'
    ...
    def add_variant(variant, quantity = 1, product_customizations = [])
    ...
    def add_context(variant, context)
      current_item = line_items.detect { |line_item| line_item.variant_id == variant.id }
      if current_item
        current_item.context = context.gsub("|", "<br>") if context
        current_item.save
      end
    end
    ...
    def after_confirm
      OrderMailer.user_confirmation_email(self).deliver
    end
